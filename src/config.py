...
class Config:
    ...
    # 本地 a-stock-data / global-stock-data toolbox 主源开关（当前为路径发现与诊断阶段）
    enable_toolbox_data_sources: bool = False
    # 实时行情数据源优先级（逗号分隔）
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            # 本地数据 toolbox；未开启时严格保持既有数据源行为。
            enable_toolbox_data_sources=os.getenv(
