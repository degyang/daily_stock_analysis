...
| `ENABLE_TOOLBOX_DATA_SOURCES` | Enables discovery of local `third_party/a-stock-data` and `third_party/global-stock-data`. Default: false. Missing, broken, or unreadable links record diagnostics and keep existing DSA providers active. Phase 1 does not change runtime routing. | `false` | Optional |
